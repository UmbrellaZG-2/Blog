...
public class ArticleController {
	...
	@GetMapping({ "", "/" })
	// 文章列表页 - 分页加载，返回的文章对象包含hasCoverImage字段，前端可根据该字段决定是否显示封面图片
    ...
    public ApiResponse<ArticleDTO> createArticle(@RequestBody ArticleDTO articleDTO) {
        logger.info("开始创建文章，标题: {}", articleDTO.getTitle());
        ...
        ArticleDTO resultDTO = dtoConverter.convertToDTO(savedArticle);
        logger.info("成功创建文章，ID: {}", savedArticle.getId());
